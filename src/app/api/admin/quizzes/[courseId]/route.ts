...
// GET /api/admin/quizzes/[courseId] - Get quizzes for 
course
...
status: authResult.status });
    ...
    const { data: quizzes, error } = await 
supabaseServer
      .from('quizzes')
      .select(`
        id,
...
quizzes' 
    ...
    const { data: modules, error: moduleError } = await 
supabaseServer
      .from('modules')
      .select('id')
      .eq('week_id', courseId); // Assuming courseId 
maps to 
week_id or adjust as needed
    if (moduleError) {
...
status: authResult.status });
    ...
    if (!title || !moduleId || !questions || 
!Array.isArray(questions)) {
      return NextResponse.json({ 
        error: 'Title, moduleId, and questions array are 
required' 
      }, { status: 400 });
    }
...
supabaseServer
    ...
    if (quizError) {
      ...
      return NextResponse.json({ error: 'Failed to 
create quiz' }, 
{ status: 500 });
    }
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      const { data: createdQuestion, error: 
questionError } = 
...
string, index: number) => ({
        ...
        const { error: answersError } = await 
supabaseServer
          .from('answers')
...
answers
    const { data: completeQuiz, error: fetchError } = 
await 
supabaseServer
      .from('quizzes')
      .select(`
      ...
      .single();
    if (fetchError) {
      console.error('Error fetching complete quiz:', 
fetchError);
    }
