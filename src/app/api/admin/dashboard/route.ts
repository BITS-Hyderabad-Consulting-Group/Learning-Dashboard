...
status: authResult.status });
    ...
    const { count: activeCourses } = await 
supabaseServer
      .from('courses')
      .select('id', { count: 'exact', head: true })
      .eq('is_active', true);
    // Get total students count
    const { count: totalStudents } = await 
supabaseServer
      .from('profiles')
      .select('id', { count: 'exact', head: true });
    // Get total enrollments count
    const { count: totalEnrollments } = await 
supabaseServer
      .from('user_course_enrollments')
      .select('id', { count: 'exact', head: true });
    // Get recent courses
    const { data: recentCourses, error: coursesError } = 
await 
supabaseServer
      .from('courses')
      .select(`
...
coursesError);
    ...
    const { data: enrollmentStats, error: 
enrollmentError } = 
...
({
      ...
      enrollments: courseEnrollmentCounts[course.id] || 
0
    })) || [];
    const dashboardData = {
      stats: {
