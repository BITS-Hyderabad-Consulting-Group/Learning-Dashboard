...
// PUT /api/admin/quizzes/[courseId]/[quizId] - Update 
quiz
...
status: authResult.status });
    ...
    if (!title && !questions) {
      return NextResponse.json({ 
        error: 'At least one field (title or questions) 
is 
required for update' 
      }, { status: 400 });
    }
...
title' }, { status: 500 });
    ...
    if (questions && Array.isArray(questions)) {
      ...
      const { data: existingQuestions } = await 
supabaseServer
        .from('questions')
        .select('id')
        .eq('quiz_id', quizId);
...
> 0) {
        const questionIds = existingQuestions.map(q => 
q.id);
...
answersError);
    ...
    const { data: quiz, error: fetchError } = await 
supabaseServer
      .from('quizzes')
      .select(`
        id,
...
// DELETE /api/admin/quizzes/[courseId]/[quizId] - 
Delete quiz
...
status: authResult.status });
    ...
    const { data: existingQuiz, error: checkError } = 
await 
supabaseServer
      .from('quizzes')
      .select('id')
