...
difficulty_level, is_active')
    ...
    if (courseError || !course) {
      return NextResponse.json({ error: 'Course not 
found' }, { 
status: 404 });
    }
    // Get weeks with modules
    const { data: weeks, error: weeksError } = await 
supabaseServer
      .from('weeks')
      .select(`
        id,
      ...
      .order('week_number')
      .order('order_in_week', { referencedTable: 
'modules' });
    if (weeksError) {
      console.error('Error fetching weeks:', 
weeksError);
...
content' }, { status: 500 });
    ...
    const transformedModules = weeks?.flatMap(week => 
...
status: authResult.status });
    ...
    if (type === 'week') {
      const { title, description, duration, weekNumber } 
= data;
      ...
      // Create new week
      const { data: newWeek, error: createError } = 
await 
supabaseServer
        .from('weeks')
        .insert([
...
createError);
      ...
      if (!weekId || !title || !contentType) {
        return NextResponse.json(
          { error: 'Week ID, title, and content type are 
required' 
},
          { status: 400 }
        );
      ...
      const { count: moduleCount } = await 
supabaseServer
        .from('modules')
        .select('id', { count: 'exact', head: true })
        .eq('week_id', weekId);
      // Create new module
      const { data: newModule, error: createError } = 
await 
supabaseServer
        .from('modules')
        .insert([
          {
            ...
            points: points || 0,
            order_in_week: orderIndex || (moduleCount || 
0) + 1,
            estimated_read_time: contentType === 
'article' ? 
duration : null,
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString()
