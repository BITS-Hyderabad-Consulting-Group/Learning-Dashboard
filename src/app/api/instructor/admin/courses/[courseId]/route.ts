...
status: 404 });
    ...
    const { count: enrollmentCount } = await 
supabaseServer
    ...
    const transformedCourse = {
      ...
      duration: `${Math.ceil((course.duration || 0) / 
60)} weeks`, 
// Convert minutes to weeks
      modules: 0, // Will be calculated from 
weeks/modules
      attendees: enrollmentCount || 0,
      prereq: course.prerequisites || 'None',
      badge_name: `${course.title} Expert`, // Generate 
badge name
      status: course.is_active ? 'active' : 'draft',
      domain: 'Business', // Default domain - you can 
add this 
      banner_url: '', // Add this field to your table if 
needed
...
supabaseServer
    ...
    if (enrollmentCount && enrollmentCount > 0) {
      return NextResponse.json(
        { error: 'Cannot delete course with active 
enrollments' },
        { status: 400 }
      );
    }
    // Delete course (this will cascade delete weeks and 
modules 
