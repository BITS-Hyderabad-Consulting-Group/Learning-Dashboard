...
const parseMarkdown = (markdown: string): string => {
  ...
  // Headers
  html = html.replace(/^### (.*$)/gim, '<h3 class="text-lg font-medium mb-3 text-gray-800">$1</h3>');
  html = html.replace(/^## (.*$)/gim, '<h2 class="text-xl font-medium mb-4 text-gray-800">$1</h2>');
  html = html.replace(/^# (.*$)/gim, '<h1 class="text-2xl font-medium mb-4 text-gray-800">$1</h1>');
  ...
  // Code blocks
  html = html.replace(/```([\s\S]*?)```/gim, '<pre class="bg-gray-100 p-3 rounded-lg my-3 text-sm overflow-x-auto"><code>$1</code></pre>');
  
  // Inline code
  html = html.replace(/`([^`]*)`/gim, '<code class="bg-gray-100 px-1 py-0.5 rounded text-sm">$1</code>');
  
  // Links
  html = html.replace(/\[([^\]]*)\]\(([^\)]*)\)/gim, '<a href="$2" class="text-teal-600 hover:text-teal-700 underline">$1</a>');
...
export function MarkdownEditor({ title, content, onTitleChange, onContentChange, placeholder = "Write your content here in Markdown..." }: MarkdownEditorProps) {
          ...
          <div className="prose prose-sm max-w-none">
            ...
            {content ? (
              ...
              <div className="text-gray-400 text-center py-12">
                ...
                <p className="text-sm mt-2">Try typing some Markdown in the editor!</p>
              </div>
            )}
          </div>
        </CardContent>
